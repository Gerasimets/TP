...
public:
	...
	void push(T2 new_value, char* new_ch = 0) // добавление элементов в конец стека
	{
	...
	void pop() // извлекаем последний(головной) элемент из стека
	{
		...
		deleted_element = last->get_value(); //присваиваем значение последнего элемента 
		...
		else // если больше одного элемента
		{
			...
			last = last->get_prev(); //в указатель на последний присваиваем предыдущий
	...
	void show() //вывод стека на экран
	{
		try
		{
			...
			while (current->get_prev() != 0) // пока не дойдем до первого элемента
			{
				cout << setprecision(4) << current->get_value() << " <- "; // выводим значение текущего
				current = current->get_prev(); // идем к началу
			}
			cout << setprecision(4) << current->get_value() << endl; // последний выводим отдельно
			cout << resetiosflags(ios_base::floatfield);
		}
		catch (const char* ex)
		{
	...
	myStack& operator = (const myStack& tmp_stack) // перегрузка оператора присваивания
	{
		...
		for (int i = tmp_stack.size - 1; i >= 0; i--)
		{
			...
			tmp_last = tmp_last->get_prev(); // временный идет от конца к началу
		}
	...
	void operator + (myStack<T2>& Stack_tmp) // сложение двух стеков
	{
		...
		else
		{
			...
			while (current_last_tmp->get_prev() != 0) // пока не дойдем до первого элемента
			{
				current_last_tmp->set_value(current_last_tmp->get_value() + current_last_tmp2->get_value()); // прибавляем к значению текущей значение второй//////////////
				current_last_tmp = current_last_tmp->get_prev(); // двигаем к началу текущую
				current_last_tmp2 = current_last_tmp2->get_prev(); // двигаем к началу вторую
			}
			current_last_tmp->set_value(current_last_tmp->get_value() + current_last_tmp2->get_value()); // к первому тоже прибавляем
	...
	void operator - (myStack<T2>& Stack_tmp) // сложение двух стеков
	{
		...
		else
		{
			...
			while (current_last_tmp->get_prev() != 0) // пока не дойдем до первого элемента
			{
				current_last_tmp->set_value(current_last_tmp->get_value() - current_last_tmp2->get_value());
				current_last_tmp = current_last_tmp->get_prev(); // двигаем к началу текущую
				current_last_tmp2 = current_last_tmp2->get_prev(); // двигаем к началу вторую
			}
			current_last_tmp->set_value(current_last_tmp->get_value() - current_last_tmp2->get_value());
	...
	void operator == (const myStack& tmp_stack)
	{
		...
		else //если размер равен, сравниваем поэлементно
		{
			...
			do
			{
				if (tmp_comprasion_1->get_value() == tmp_comprasion_2->get_value())
				{
					i--;
				}
				else
				...
				tmp_comprasion_1 = tmp_comprasion_1->get_prev(); //двигаемся по элементам
				tmp_comprasion_2 = tmp_comprasion_2->get_prev();
			} while (i != 0);
		}
